    ...
    private func writeGradientPreviewBackground(to url: URL, width: Int, height: Int) throws {
        ...
        for y in 0..<height {
            ...
            for x in 0..<width {
                ...
                let top = mixColor((46, 61, 87), (143, 181, 212), fx)
                let bottom = mixColor((244, 186, 158), (79, 123, 181), fx)
                let base = mixColor(top, bottom, fy)
                let vignette = 0.84 + 0.16 * (1 - min(1, hypot(fx - 0.5, fy - 0.5)))
                pixels.append(UInt8(max(0, min(255, Double(base.0) * vignette))))
                pixels.append(UInt8(max(0, min(255, Double(base.1) * vignette))))
                pixels.append(UInt8(max(0, min(255, Double(base.2) * vignette))))
            }
        }
