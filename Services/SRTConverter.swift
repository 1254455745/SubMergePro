...
enum SRTConverter {
    ...
    static func makeSampleASS(assURL: URL, appearance: SubtitleAppearance, text: String) throws {
        ...
        let ass = assHeader(appearance: appearance) + dialogueLines(
            start: "0:00:00.00",
            end: "0:00:10.00",
            text: escapedText,
    ...
    private static func convertSRTToASS(srtURL: URL, assURL: URL, appearance: SubtitleAppearance) throws {
        ...
        for block in blocks {
            ...
            let text = sanitizeDialogueText(textLines.joined(separator: "\\N"))
            ass += dialogueLines(start: start, end: end, text: text, appearance: appearance)
        }
    ...
    private static func restyleASS(assURL: URL, outputURL: URL, appearance: SubtitleAppearance) throws {
        ...
        for rawLine in content.replacingOccurrences(of: "\r\n", with: "\n").split(separator: "\n", omittingEmptySubsequences: false) {
            ...
            let text = sanitizeDialogueText(fields[9...].joined(separator: ","), removingASSOverrides: true)
            output += dialogueLines(start: start, end: end, text: text, appearance: appearance)
        }
    ...
    private static func dialogueLines(
        start: String,
        end: String,
        text: String,
        appearance: SubtitleAppearance
    ) -> String {
        let textLayer = SubtitleStyleBuilder.dialoguePrefix(from: appearance) + text
        guard appearance.backgroundEnabled else {
            return "Dialogue: 0,\(start),\(end),Default,,0,0,0,,\(textLayer)\n"
        }
        let backgroundLayer = SubtitleStyleBuilder.backgroundDialoguePrefix(from: appearance) + text

        return """
        Dialogue: 0,\(start),\(end),Background,,0,0,0,,\(backgroundLayer)
        Dialogue: 1,\(start),\(end),Default,,0,0,0,,\(textLayer)
